...
public class SpecialProductList : MonoBehaviour {
    public GameObject header, body, bodycontent, downmenu, modify, addcategory, addproduct, detail;
    public ScrollingTab tab;
    public GameObject item, upPanel;
    public GameObject noitem;
    public GameObject deleteWindow, loading, addbtn, addback, addbackground;
	...
	void Start () {
        ...
        PlayerPrefs.SetString("product_time", Utils.GetCurTime());
    }
    ...
    IEnumerator GetResults()
    {
        ...
        WWW www = new WWW(Global.URL_SPECIALPRODUCTLIST + 0 + "?category="+keys[curSel]);
        yield return www;
    ...
    {
        using (var w = UnityWebRequest.Delete(Global.URL_SPECIALPRODUCTDELETE + deleteId))
        {
            ...
            {
                StartCoroutine(Utils.ShowMessage("삭제되었습니다.", MessageType.WARNING));
            }
    ...
    {
        detail.GetComponent<ProductDetail>().SetValues(true, item, "특가상품상세보기");
    }
}
